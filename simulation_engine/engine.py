...
def run_round(state: GameState) -> tuple[bool, bool]:
    ...
    if mission_name is None:
        ...
        if state.skip_next_complication:
            complication = ComplicationCard.get(ComplicationCardName.CALM_BREEZE)
            ...
            first_complication_card, second_complication_card = ComplicationCard.get(first_complication_name), ComplicationCard.get(second_complication_name)
            ...
            complication = ComplicationCard.get(draw_complication(state))
        ...
        if success:
            ...
            point_penalty = VolcanoCard.get(state.pending_volcano_card).mission_point_penalty if state.pending_volcano_card is not None else 0
        ...
        # Step 5 — Exhaustion
        extra_exhaustion = VolcanoCard.get(state.pending_volcano_card).extra_exhaustion_rounds if state.pending_volcano_card is not None else 0
    ...
    # Step 6 — Gather
    gather_yields_zero = VolcanoCard.get(state.pending_volcano_card).gather_yields_zero if state.pending_volcano_card is not None else False
